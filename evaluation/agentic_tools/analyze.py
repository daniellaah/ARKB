...
def analyze(output, grading, destination):
    ...
    if protocol['phase'] != 'core' or len(original) != 5460:
    ...
    summaries = {}
    for ds in ('browsecomp-plus', 'fiqa', 'nfcorpus', 'musique'):
        rows = [r for r in scored if r['schedule']['dataset'] == ds]
        arms = {}
        for arm in ARMS:
        ...
        summary = {'arms': arms, 'attempts': len(rows), 'evidence_comparisons': {}, 'a_all_cost_ratios': ratios}
        ...
        if ds == 'browsecomp-plus':
            summary['answer_success'] = paired_comparisons(query_units(rows, 'answer_success'), primary=True)
            summary['quality_status'] = 'provisional_pending_independent_human_review'
        if ds == 'musique':
            pairs = musique_pairs(rows)
            summary['musique_metrics'] = {metric: paired_comparisons(query_units(pairs, metric)) for metric in
    ...
    summary = {'status': 'automated_analysis_complete_review_pending', 'quality_status': 'provisional',
               'datasets': summaries, 'registered_attempts': len(scored), 'trace_checks': checks,
