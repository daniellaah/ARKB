...
def main():
    ...
    verification_path='verification-post-scale/test-summary.json'
    tests=read(verification_path) if (V/verification_path).exists() else None
...
The diagram shows the shared ranking operation and the separate evaluation projection. Runtime default candidate depth is 20 per leg; this frozen benchmark captures 500 per leg.''')
    ...
    if (V/'browsecomp-length-audit.json').exists():
        lengths=read('browsecomp-length-audit.json');chars=lengths['body_characters']
        add(f"The full BrowseComp corpus contains {chars['total']:,} body characters. Median document length is {chars['p50']:,.0f} characters, p99 is {chars['p99']:,.0f}, and the maximum is {chars['max']:,}; {lengths['counts']['1000000']:,} documents have at least one million characters. This operational audit reads corpus lengths only, without qrels or query inspection. It characterizes the ingestion workload; no long document was removed or truncated. See [length audit](../evaluation/phase-c/v1/browsecomp-length-audit.json).")
    ...
    for ds in ('nfcorpus','fiqa','browsecomp-plus'):
        ...
        if (V/f'validation/{ds}-cost.json').exists():
            cost=read(f'validation/{ds}-cost.json')
            add(table(['Observed retrieval leg','Mean ms','p50 ms','p95 ms','Mean returned chunks'],[
                [leg,*[m[k] for k in ('mean_ms','p50_ms','p95_ms','mean_returned_chunks')]] for leg,m in cost['arms'].items()]))
            add('These are observed single-capture search timings, including query embedding for Semantic and result hydration, excluding offline fusion and index preparation. They are not repeated isolated latency measurements or default-depth/Agent latency estimates. Other corpus preparation could share host resources.')
    ...
    add('A later corpus-only preparation probe on 26 BrowseComp documents reduced CPU preparation from 12.288 to 4.419 seconds with eight processes (2.78 times as fast on that sample), preserving every chunk field, input and token count. Full resumable input preparation runs separately and must match the original complete input digest before replacing any unfinished serial preparation. Twelve operational equivalence/recovery checks passed without model or retrieval calls. A direct eight-slot inference probe gave no throughput improvement and differed numerically from stored vectors; it was not adopted. This CPU optimization changes operational cache staging and recovery only; it does not establish an end-to-end speedup or accelerate the normal production builder. Full-plan/cutover status and limitations are recorded in the execution note.')
    add('A subsequent user-requested GPU throughput investigation measured native MLX float16 batching at 25.02 inputs/second versus original Ollama Q8_0 at 16.47 on the same fixed 256 FiQA inputs, with three rotated repetitions. TEI Metal float16 was slower. Alternative vectors differ numerically from production; no retrieval quality comparison or production adoption occurred. The 51.9% MLX sample throughput gain changes backend/precision and excludes an MLX HTTP server, so it is not an unchanged-model full-corpus speedup claim. See [GPU throughput investigation](embedding-throughput-investigation.md).')
