...
class ExternalDataset:
    ...
    def save(self, output, *, materialize=False):
        ...
        write_json(output/'manifest.json', {'schema':'arkb-external-v1','name':self.name,
            ...
            'ranking_unit':'original corpus ID','exposure':self.provenance.get('exposure','public-development'),'release_gold':False,
    ...
    def materialize(self, directory, *, omit_empty=False):
        ...
        for n in self.notes():
            if omit_empty and not (n.title+n.content).strip():continue
    ...
    def verify_materialized(self, directory, *, omit_empty=False):
        ...
        if omit_empty:notes=[n for n in notes if (n.title+n.content).strip()]
...
def import_beir(inputs, name, *, query_limit=None):
    ...
    required={f'{name}/README.md',f'{name}/corpus/corpus-00000-of-00001.parquet',
              f'{name}/queries/queries-00000-of-00001.parquet',f'{name}-qrels/test.tsv'}
    ...
    if len(queries)!=len(query_rows):raise ValueError('Duplicate official query ID.')
    ...
    return ExternalDataset(name,[{'id':str(d['_id']),'title':d.get('title') or '', 'text':d['text']} for d in docs],
        {q:queries[q] for q in selected},{q:qrels[q] for q in selected},{},
        {'split':'test','scope':'full BEIR corpus','files':files,'query_limit':query_limit,'sample_seed':20260910}).validate()
...
def import_public(inputs, name, *, query_limit=None):
    ...
    inputs=Path(inputs); downloads=json.loads((inputs/'downloads.json').read_text())
    if name not in ('bright-stackoverflow','bright-robotics'):raise ValueError('Unknown external dataset.')
    ...
    docs=[{'id':str(r['id']),'title':'','text':r['content']} for r in _parquet(inputs/f'bright-pro/documents/{domain}.parquet')]
    examples=_parquet(inputs/f'bright-pro/examples/{domain}.parquet')
    queries={str(r['id']):r['query'] for r in examples};qrels=defaultdict(dict);aspects={}
    for row in examples:
        for d in row['gold_ids']:
            if d in qrels[str(row['id'])]:raise ValueError('Duplicate qrel.')
    ...
    selected=sorted(qrels)
    if query_limit is not None:selected=sample_ids(selected,query_limit)
    return ExternalDataset(name,docs,{q:queries[q] for q in selected},{q:qrels[q] for q in selected},
        {q:aspects[q] for q in selected if q in aspects},
        {'split':domain,'scope':'full per-domain Bright-Pro corpus','files':files,'query_limit':query_limit,'sample_seed':20260910}).validate()
