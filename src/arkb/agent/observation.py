...
class AgentObserver:
    ...
    def __init__(self, *, budget=None, counter=None, counter_identity=None, clock=perf_counter):
        ...
        if counter is not None and (
            not callable(counter) or not isinstance(counter_identity, str) or not counter_identity.strip()
        ):
    ...
    def start(self):
        if self.started is not None:
            raise ValueError('Create a fresh observer for every run.')
    ...
    def request_tools(self, turn, calls):
        ...
        for call in calls:
            event = {
                ...
                'arguments': deepcopy(call.function.arguments),
                'status': 'requested',
                'executed': False,
                'started_ms': None,
                'elapsed_ms': None,
                'raw_result': None,
                'error': None,
                'delivered_to_conversation': False,
                'submitted_to_model': False,
                'returned_evidence_tokens': None,
                'delivered_evidence_tokens': None,
                'skip_reason': None,
            }
    ...
    def permit_tool(self, event):
        ...
        if self.budget:
            ...
            for key, count in checks:
                applies = (
                    key == 'max_tool_calls'
                    or (key == 'max_query_calls' and event['name'] in ('match', 'search'))
                    or (key == 'max_read_calls' and event['name'] == 'read')
                )
    ...
    def remaining_evidence(self):
        if not self.budget or self.budget.max_evidence_tokens is None:
            return None
    ...
    def end_tool(self, event, result, *, references=None):
            ...
            if kept < len(bound):
                ...
                if kept == 0 or event['name'] == 'read':
                    if remaining < limit / 10:
                        self.reason = 'max_evidence_tokens'
                    return False
        ...
        for h in bound:
            identity = json.dumps(
                {key: h.get(key) for key in ('source', 'document_revision', 'start_char', 'end_char', 'content')},
                sort_keys=True,
                ensure_ascii=False,
            )
    ...
    def fail(self, error, stage, event=None):
        ...
        target = (
            ...
            else (self.models[-1] if stage in ('model_request', 'model_protocol') and self.models else None)
        )
        if target is not None:
            target.update(status='fatal_error', error=detail)
            if target['started_ms'] is not None:
                target['elapsed_ms'] = self.elapsed() - target['started_ms']
    ...
    def finish(self, stop_reason):
        ...
        def usage(key):
            ...
            return {
                ...
                'defined_requests': len(known),
                'requests': len(values),
            }

        return deepcopy(
            {
                'schema_version': 'arkb-agent-observation-v2',
                ...
                'evidence': {
                    'returned_tokens': sum(e['returned_evidence_tokens'] or 0 for e in self.tools)
                    if self.counter
                    else None,
                    'delivered_tokens': sum(e['delivered_evidence_tokens'] or 0 for e in self.tools)
                    if self.counter
                    else None,
                    'unique_exact_excerpt_tokens': sum(self._unique.values()) if self.counter else None,
                },
                'limits': [
                    'Exact execution is bounded and cancellable; other in-flight provider work uses transport timeouts and a cooperative run deadline.',
