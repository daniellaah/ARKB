...
class ExternalDataset:
    ...
    def validate(self):
        ...
        if any(
            set(d) != {'id', 'title', 'text'} or not isinstance(d['title'], str) or not isinstance(d['text'], str)
            for d in self.corpus
        ):
            raise ValueError('Only document ID, title and text belong in corpus.')
        if not self.queries or any(
            not isinstance(q, str) or not q or not isinstance(t, str) or not t.strip() for q, t in self.queries.items()
        ):
    ...
    def notes(self):
        return [
            Note(
                title=' '.join(d['title'].splitlines()).strip(),
                content=d['text'].strip(),
                source=opaque_source(self.name, d['id']),
            )
            for d in self.corpus
        ]
    ...
    def records(self):
        notes = self.notes()
        return [ChunkRecord.from_note(c, note=n, vault_id=self.name) for n, c in zip(notes, whole_note_chunks(notes))]
    ...
    def save(self, output, *, materialize=False):
        ...
        write_json(output / 'queries.json', self.queries)
        write_json(output / 'qrels.json', self.qrels)
        write_json(output / 'aspects.json', self.aspects)
        if materialize:
            self.materialize(output / 'corpus')
        write_json(
            output / 'manifest.json',
            {
                'schema': 'arkb-external-v1',
                'name': self.name,
                'corpus_count': len(self.corpus),
                'query_count': len(self.queries),
                'provenance': self.provenance,
                'query_ids': list(self.queries),
                'normalization': 'title lines joined; body stripped; raw corpus preserved',
                'ranking_unit': 'original corpus ID',
                'exposure': self.provenance.get('exposure', 'public-development'),
                'release_gold': False,
                'files': {
                    n: digest(output / n) for n in ('corpus.jsonl', 'queries.json', 'qrels.json', 'aspects.json')
                },
            },
        )

    def materialize(self, directory, *, omit_empty=False):
        directory = Path(directory)
        directory.mkdir(parents=True, exist_ok=False)
        for n in self.notes():
            if omit_empty and not (n.title + n.content).strip():
                continue
            (directory / n.source).write_text('# ' + n.title + '\n' + n.content, encoding='utf-8')

    def verify_materialized(self, directory, *, omit_empty=False):
        directory = Path(directory)
        notes = self.notes()
        if omit_empty:
            notes = [n for n in notes if (n.title + n.content).strip()]
...
def load_external(directory):
    ...
    data = ExternalDataset(
        m['name'],
        read_jsonl(directory / 'corpus.jsonl'),
        *[json.loads((directory / n).read_text()) for n in ('queries.json', 'qrels.json', 'aspects.json')],
        m['provenance'],
    ).validate()
    if (
        len(data.corpus) != m['corpus_count']
        or list(data.queries) != m['query_ids']
        or len(data.queries) != m['query_count']
    ):
...
def import_beir(inputs, name, *, query_limit=None):
    """Normalize official BEIR corpus/query Parquet plus test TSV judgments."""
    inputs = Path(inputs)
    downloads = json.loads((inputs / 'downloads.json').read_text())
    required = {
        f'{name}/README.md',
        f'{name}/corpus/corpus-00000-of-00001.parquet',
        f'{name}/queries/queries-00000-of-00001.parquet',
        f'{name}-qrels/test.tsv',
    }
    files = {n: v for n, v in downloads['files'].items() if n.startswith((name + '/', name + '-qrels/'))}
    if not required <= set(files):
        raise ValueError('Required download hashes are missing.')
    ...
    docs = _parquet(inputs / f'{name}/corpus/corpus-00000-of-00001.parquet')
    query_rows = _parquet(inputs / f'{name}/queries/queries-00000-of-00001.parquet')
    queries = {str(r['_id']): r['text'] for r in query_rows}
    if len(queries) != len(query_rows):
        raise ValueError('Duplicate official query ID.')
    with (inputs / f'{name}-qrels/test.tsv').open() as f:
        judgments = list(csv.DictReader(f, delimiter='\t'))
    ...
    if query_limit is not None:
        selected = sample_ids(selected, query_limit)
    return ExternalDataset(
        name,
        [{'id': str(d['_id']), 'title': d.get('title') or '', 'text': d['text']} for d in docs],
        {q: queries[q] for q in selected},
        {q: qrels[q] for q in selected},
        {},
        {
            'split': 'test',
            'scope': 'full BEIR corpus',
            'files': files,
...
def import_public(inputs, name, *, query_limit=None):
    if name in ('scifact', 'nfcorpus', 'fiqa'):
        return import_beir(inputs, name, query_limit=query_limit)
    inputs = Path(inputs)
    downloads = json.loads((inputs / 'downloads.json').read_text())
    if name not in ('bright-stackoverflow', 'bright-robotics'):
        raise ValueError('Unknown external dataset.')
    files = {n: v for n, v in downloads['files'].items() if n.startswith('bright-pro/')}
    ...
    if not required <= set(files):
        raise ValueError('Required download hashes are missing.')
    for n, v in files.items():
        if digest(inputs / n) != v['sha256']:
            raise ValueError('Downloaded file changed: ' + n)
    docs = [
        {'id': str(r['id']), 'title': '', 'text': r['content']}
        for r in _parquet(inputs / f'bright-pro/documents/{domain}.parquet')
    ]
    ...
    for row in examples:
        for d in row['gold_ids']:
            if d in qrels[str(row['id'])]:
                raise ValueError('Duplicate qrel.')
            qrels[str(row['id'])][d] = 1
    ...
    return ExternalDataset(
        ...
        {
            'split': domain,
            'scope': 'full per-domain Bright-Pro corpus',
            'files': files,
            'query_limit': query_limit,
            'sample_seed': 20260910,
        },
...
def musique_groups(inputs, *, groups=100):
    ...
    return [r for q in selected for r in sorted(pairs[q], key=lambda r: not r['answerable'])], {
        'archive': manifest['files']['musique.zip'],
        'available_rows': len(rows),
        'available_complete_pairs': len(pairs),
        'selected_group_ids': selected,
        'sample_seed': 20260910,
        'scope': 'per-question supplied context',
    }


def rank_metrics(qrels, ranking, *, ks=(10, 100)):
    if len(ranking) != len(set(ranking)):
        raise ValueError('Duplicate native IDs in ranking.')
    if not ks or any(type(k) is not int or k < 1 for k in ks):
        raise ValueError('Invalid cutoff.')
    if any(type(v) is not int or v < 0 for v in qrels.values()):
        raise ValueError('Invalid qrels.')
    positives = {d for d, v in qrels.items() if v > 0}
    ideal = sorted(qrels.values(), reverse=True)
    result = {}
    for k in ks:
        top = ranking[:k]
        dcg = sum(qrels.get(d, 0) / math.log2(i + 2) for i, d in enumerate(top))
        ...
        result[f'mrr@{k}'] = (
            next((1 / (i + 1) for i, d in enumerate(top) if d in positives), 0.0) if positives else None
        )
    return result


def aspect_metrics(aspects, ranking, *, k=10, alpha=0.5):
    if not 0 <= alpha <= 1 or type(k) is not int or k < 1 or len(ranking) != len(set(ranking)):
        raise ValueError('Invalid aspect metric inputs.')
    if not aspects:
        return {'alpha_ndcg@' + str(k): None, 'aspect_recall@' + str(k): None}
    ...
    for i, d in enumerate(ranking[:k]):
        if d in mapping:
            a = mapping[d]
            dcg += weights[a] * (1 - alpha) ** counts[a] / math.log2(i + 2)
            counts[a] += 1
    gains = sorted(
        (weights[a['id']] * (1 - alpha) ** i for a in aspects for i in range(len(a['supporting_docs']))), reverse=True
    )
    ideal = sum(g / math.log2(i + 2) for i, g in enumerate(gains[:k]))
    return {
        'alpha_ndcg@' + str(k): dcg / ideal if ideal else None,
...
def score_ranking(dataset, qid, ranking, *, ks=(10, 100)):
    ...
    return {
        **rank_metrics(dataset.qrels[qid], ranking, ks=ks),
...
def reference_metrics(qrels, rankings, *, ks=(10, 100)):
    ...
    if set(qrels) != set(rankings):
        raise ValueError('Reference run must include every query.')
    ...
    return {
        q: {
            f'{m}@{k}': result.get(q, {}).get(f'{"ndcg_cut" if m == "ndcg" else "recall"}_{k}', 0.0)
