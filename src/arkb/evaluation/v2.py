...
class EvalDataset:
    ...
    def provisional(self):
        return any(
            row['annotation_status'] != 'reviewed' for row in (*self.cases, *self.evidence.values(), *self.qrels)
        )
...
def load_dataset(directory: Path, *, notes_dir: Path, allow_provisional=False) -> EvalDataset:
    ...
    for doc in manifest['corpus']:
        ...
        if (
            not _text(source)
            or source in recorded
            or source not in notes
            or not (notes_dir / source).resolve().is_relative_to(notes_dir.resolve())
        ):
            raise ValueError('Invalid, duplicate, or external corpus source.')
        recorded.add(source)
        note = notes[source]
        if (
            note.document_revision != doc['document_revision']
            or hashlib.sha256(note.content.encode()).hexdigest() != doc['body_sha256']
        ):
    ...
    for span in _read_rows(directory / 'evidence.jsonl'):
        ...
        if (
            span['document_revision'] != note.document_revision
            or note.content[start:end] != span['quote']
    ...
    for case in _read_rows(directory / 'queries.jsonl'):
        ...
        if any(
            not _text(case[k])
            for k in ('id', 'intent_family_id', 'query', 'query_language', 'query_origin', 'expected_behavior')
        ):
        ...
        if case['task_type'] == 'evidence_gap' and case['answerability'] not in (
            'partial',
            'unanswerable',
            'needs_clarification',
        ):
        ...
        for facet in case['evidence_requirements']:
            ...
            if (
                not _text(facet['id'])
                or facet['id'] in facet_ids
                or type(facet['critical']) is not bool
                or type(facet['weight']) not in (int, float)
                or not math.isfinite(facet['weight'])
                or facet['weight'] <= 0
            ):
        ...
        if (
            case['answerability'] in ('answerable', 'partial')
            and not case['evidence_requirements']
        ...
        if case['task_type'] == 'exact_lookup':
            ...
            if (
                not _text(pattern['query'])
                or type(pattern['regex']) is not bool
                or type(pattern['case_sensitive']) is not bool
                or pattern['target'] not in ('content', 'source')
            ):
    ...
    for rel in _read_rows(directory / 'qrels.jsonl'):
        ...
        if (
            key in pairs
            or key[0] not in ids
            or key[1] not in notes
            or type(rel['grade']) is not int
            or not 0 <= rel['grade'] <= 3
        ):
    ...
    for case in cases:
        ...
        if case['task_type'] == 'no_retrieval' and any(
            q == case['id'] and grade > 0 for (q, s), grade in qrel_map.items()
        ):
...
def ranking_scores(qrels: dict[str, int], ranked: list[str], *, k=10, rel_threshold=2):
    ...
    return {
        f'recall@{k}': len(relevant.intersection(top)) / len(relevant) if relevant else None,
        f'mrr@{k}': next((1 / i for i, s in enumerate(top, 1) if s in relevant), 0.0) if relevant else None,
        ...
        'unassessed_returned_count': sum(s not in qrels for s in top),
    }
...
def observation_intervals(observations: list[dict], dataset: EvalDataset):
    ...
    for hit in observations:
        ...
        start, end = hit.get('start_char'), hit.get('end_char')
        ...
        if (
            ...
            or source not in dataset.bodies
...
def unique_evidence_tokens(observations, dataset, *, counter):
    ...
    for source, ranges in intervals.items():
        ...
        for start, end in merged:
            ...
            spans += 1
    return {
        'union_reference_tokens': tokens,
        'merged_intervals': spans,
        'documents': len(intervals),
...
def evidence_scores(case: dict, observations: list[dict], dataset: EvalDataset):
    ...
    satisfied = {
        f['id']: any(all(fractions[e] == 1 for e in alt) for alt in f['alternatives'])
    ...
    return {
        'evidence_coverage': sum(f['weight'] * satisfied[f['id']] for f in case['evidence_requirements']) / weight
        if weight
        else None,
        'all_critical_facets_covered': all(critical) if critical else None,
        'facet_satisfied': satisfied,
        'span_coverage': fractions,
        'rejected_observations': rejected,
        'annotation_status': 'provisional' if dataset.provisional else 'reviewed',
        'answer_correct': None,
        'grounded_task_success': None,
    }
...
def dataset_report(dataset):
    return {
        ...
        'corpus_documents': len(dataset.bodies),
        'provisional': dataset.provisional,
