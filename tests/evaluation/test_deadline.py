...
def test_blocked_call_retains_failure_observation(stage):
    class Tools:
        def tool_definitions(self): return TOOL_DEFINITIONS
    ...
    with evaluation_deadline(0.05):
        result = run_agent('fixture', client=client, tools=Tools(), model='fixture', observer=observer)
    assert result.final.error['type'] == 'EvaluationDeadlineExceeded'
